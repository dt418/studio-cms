...
- `opencode.json` enables Astro docs, shadcn, and GitHub MCP servers; use current docs for Astro/shadcn/library API questions instead of relying on memory.
...
- Start a multi-agent task with `pnpm harness:orchestrate -- --role planning --runtime <codex|claude|pi|omp|opencode>`. Standard routing uses Sol High for spec/planning, Luna xhigh for implementation, Terra High for review/QA/test/advisor, Luna Medium for documentation sync/exploration, Terra Medium for docs research, and Luna Low for observation. Simple implementation may use Luna Medium; complex planning uses Astra Medium; complex implementation/review/QA/test/advisor and high-risk work escalate to Sol High. `documentation-sync` runs after QA/review, updates factual documentation/context/handoff artifacts, and does not change production code. Escalate high-risk review with `--role reviewer --risk high` for authentication, authorization, data migration, payment, or security changes.
