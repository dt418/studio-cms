...
For multi-agent work, run `pnpm harness:orchestrate -- --runtime claude`, use `.harness/` for task state and verification evidence, and run `pnpm harness:context` when source, workspace, instruction, skill-lock, or harness files change. On a fresh clone, run `pnpm harness:skills:sync` followed by `pnpm harness:skills:verify` before relying on Caveman skills.
